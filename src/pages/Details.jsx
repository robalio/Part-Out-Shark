...
function Details({ listings, onDelete, onUpdate, savedIds, onToggleSaved }) {
    ...
    function handleDeletePart(partId) {
        const updatedListing = {
            ...listing,
            parts: listing.parts.filter(p => p.id !== partId)
        };
    ...
    function handleAddPart() {
        ...
        const updatedListing = {
            ...listing,
            parts: [...listing.parts, { id: crypto.randomUUID(), name: newPart.trim() }]
        };
        onUpdate(updatedListing);
        setNewPart('');
    }
                ...
                <Col md={7}>
                    ...
                    <div className="mt-3">
                        <div className="d-flex justify-content-between align-items-center mb-1">
                            <h3 className="mb-0">Description</h3>
                            {isOwner && !editingDescription && (
                                <Button variant="outline-secondary" size="sm" onClick={handleStartEditing}>
                                    Edit
                                </Button>
                            )}
                        </div>
                        {editingDescription ? (
                            ...
                            <p style={{ whiteSpace: 'pre-wrap' }}>
                                {listing.description || <span className="text-muted">No description available for this post.</span>}
                            </p>
                        )}
                    </div>
                </Col>
